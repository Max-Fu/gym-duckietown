...
class ActorCNN(nn.Module):
    def __init__(self, action_dim, max_action, prior_dim):
        ...
        self.lin1 = nn.Linear(flat_size + prior_dim, 512)
    ...
    def forward(self, x, prior):
        ...
        x = self.dropout(x)
        x = self.lr(self.lin1(torch.cat([x, prior], 1))) # can also try addition / more layers
...
class CriticCNN(nn.Module):
    def __init__(self, action_dim, output_activation=None):
...
class PriorCNN(nn.Module):
    def __init__(self, prior_dim, output_activation=None):
        ...
        self.lin1 = nn.Linear(flat_size, 256)
        self.lin2 = nn.Linear(256, prior_dim)
    ...
    def forward(self, states):
        ...
        x = x.reshape(x.size(0), -1)  # flatten
        x = self.lr(self.lin1(x))
        x = self.output_activation(self.lin2(x))  
        return x
...
class RCRL(object):
    ...
    def __init__(self, state_dim, action_dim, max_action, prior_dim, lr_actor=1e-4, lr_critic=1e-3, lr_prior=1e-4):
        ...
        self.flat = False
        self.actor = ActorCNN(action_dim, max_action, prior_dim).to(device)
        self.actor_target = ActorCNN(action_dim, max_action, prior_dim).to(device)
        ...
        self.prior_regressor = PriorCNN(prior_dim, output_activation='sigmoid').to(device)
    ...
    def predict(self, state):
        ...
        prior_prediction = self.prior_regressor(state)
        return self.actor(state, prior_prediction).cpu().data.numpy().flatten()
    ...
    def train(self, replay_buffer, iterations, batch_size=64, discount=0.99, tau=0.001):
        ...
        for it in range(iterations):
            ...
            prior_t = torch.FloatTensor(sample["additional"]).to(device)
            prior_t = torch.reshape(prior_t, (-1, self.prior_dim))
            # Compute prior and optimize prior network 
            pred_additional = self.prior_regressor(state)
            prior_loss = F.mse_loss(pred_additional, prior_t)
            ...
            target_Q = self.critic_target(next_state, self.actor_target(next_state, prior_t))
            target_Q = reward + (done * discount * target_Q).detach()

            # Get current Q estimate
            current_Q = self.critic(state, action) # this can be additional_t
            ...
            # Compute actor loss
            actor_loss = -self.critic(state, self.actor(state, prior_t)).mean()
